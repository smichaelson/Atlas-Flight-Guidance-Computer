...
```c
/* One-time commissioning operation; preserve the test transcript. */
...
Monitor the underlying UART `dropped_bytes`, HAL error, and restart counters as well. RTS/CTS prevents many overruns but cannot repair a blocked foreground consumer or wrong pin mapping.
