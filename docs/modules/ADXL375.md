...
`AtlasIO` polls DATA_READY every 2 ms and publishes successful samples in `AtlasRtosSnapshot.adxl375`. Require `ATLAS_RTOS_VALID_ADXL375`, check `adxl375_status`, and apply an application-approved age limit to `timestamp_ms`. After scheduler start, application code must not call the direct API or touch the shared SPI2 bus; the direct example below is for an isolated pre-scheduler driver test.
...
Never convert a failed read into a zero-g sample. A stale-data deadline should be added by the future acquisition service.
...
4. Capture at least 100 data-ready polls and bursts; confirm 400 Hz timing within the oscillator/host polling tolerance.
