...
TIM15 ownership, opposite PWM modes, frequency bounds and ordinary timed stop pass deterministic host tests; the complete target image builds without warnings. The stop comparison is source-reviewed for wrap safety, but the existing buzzer test does not cross a tick wrap. Physical frequency, phase, stopped levels, differential voltage, sound, EMC and worst-case delayed-service duration remain bench-pending.
