...
The EXTI callback only increments the coalesced INT1 counter. `AtlasIO` reacts to that counter and requests a 5 ms fallback poll, then publishes only when both accelerometer and gyroscope data are ready. Other I/O work can delay service; coalescing and register polling do not guarantee every 240 Hz sample is retained. Read `AtlasRtosSnapshot.lsm6dsv16b` only with `ATLAS_RTOS_VALID_LSM6DSV16B`, an `ATLAS_OK` status, and a checked sample age. The direct example below is pre-scheduler test code; it is not a supported call from `AtlasRtos_ApplicationStep()`.
