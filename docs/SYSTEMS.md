...
The 1.2.2 diagnostic release retains the ADC correction, startup audio, revised melody and a separate ServoBench profile to the browser Ground Station and UID-bound software DFU. See the [operator guide](GROUND_STATION.md) and [three-pass review](GROUND_STATION_REVIEW.md) for actual board and remaining qualification evidence. Software entry, physical UID match and verified programming have been exercised on this board. ROM 0x92 returned directly once, but the latest update required a battery/USB cold restart with BOOT0 LOW; reliable warm return remains unqualified. ServoBench now uses the owner-selected 8.55 V PWM ceiling; other rail bands and meter comparisons no longer gate that profile. The march bridge and phrase timing are revised. The normal application has no USB maintenance parser.
...
| NEO-M9N GNSS/GPS + PPS | USART stale-RX preflight; UBX identity before PPS; RAM-only 10 Hz NAV-PVT configuration/readback; bounded parser recovery; explicit failed-probe retry and stage/HAL diagnostics | September 11 receiver identity/RAM configuration and NAV reception passed; antenna/fix/accuracy/UTC and measured PPS remain pending |
...
| ADC / rails / continuity | Dedicated output owner calibrates ADC1/3; ten scaled ranks, separate VREF/temperature conversions, validity and continuous snapshots; retained ADC3 phase/channel/raw/HAL diagnostics | The 1.2.1 reference correction restores all ten ranks; September 11 live 3V3/PWM averages agree with DMM. Divider calibration, sample noise/ripple, settling/tolerances, diagnostic-current limits and physical-arm/continuity thresholds remain unqualified |
