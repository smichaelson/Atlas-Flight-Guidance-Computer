...
|---|---|
| **Bringup / BringupRelease** | Isolated diagnostic RTOS application; USB JSON dashboard, explicit once-per-boot probes and SD tests. No control hook; PWM/pyro remain inhibited even through normal output APIs. Missing modules are reported without the normal required-sensor reset loop |
...
Use **[startup](startup.md)** for the new PCB. It centralizes power/BOOT/reset/USB DFU, fixtures, dashboard and expected evidence. Diagnostic software coverage includes sensors, GNSS/PPS, BLE, SD/RTC, voltage/input monitoring, indicators, bounded logic GPIO and optional radio/expansion tests. Power regulation and connector waveforms still need instruments; PWM/pyro need separate inert qualification. The PDF/USB manufacturing-polarity conflict is an as-built inspection gate before USB commissioning.
...
| BNO085 | I2C1 SH-2/SHTP identity plus four reports; shared TIM2 start corrected | End-to-end SHTP/reset/report behavior; fitted firmware and calibration; no full host SH-2 startup test |
| NEO-M9N GNSS/GPS + PPS | UART UBX identity; RAM-only 10 Hz NAV-PVT configuration/readback; bounded parser recovery; TIM2 capture | Real receiver/baud/configuration, antenna, fix/accuracy/UTC quality and measured PPS |
...
| RGB LED | Binary logical masks; startup status and queued runtime set | Physical green/blue mapping unresolved; not a runtime fault annunciator |
...
| ADC / rails / continuity | Dedicated output owner calibrates ADC1/3; ten scaled ranks, separate VREF/temperature conversions, validity and continuous snapshots | Reference/dividers/settling/tolerances, diagnostic-current limits, calibrated thresholds and physical arm proxy |
